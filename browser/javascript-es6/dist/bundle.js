...
let browserProxy = browser; // let browserProxy = chrome;
...
async function inputUpdated(event) {
  ...
  switch (eventListener) {
    case "hex":
      ...
      if (chunks.length > 0 && chunks[chunks.length - 1].length == 2 && chunks.length * 3 <= maxLength && oldValue.length < event.target.value.length) {
        chunks.push("");
      }
...
async function logout() {// TODO: THIS
}
...
async function deleteKey(key, showWarning) {
  ...
  return fetch(baseURL + "/yubikeys/delete-key/?key_id=" + key.id, {
    ...
    if (data.response === "success") {
      ...
      throw new Error(data.reason); // displayMessage("Key delete failed: " + data.reason, "error");
      // unloadKeyData(key);
    } else {
      throw new Error(data); // displayMessage("Key unable to delete with response: " + data, "error");
    }
...
async function addUserElements(pageId) {
  ...
  } else {//TODO: Add username and key elements
  }
  ...
  if (currentUsername == null || currentToken == null) {
    var savedUser = browser.storage.sync.get({
      "username": null,
      "token": null
    }); // user = {"username": currentUsername, "token": currentToken};

    Promise.resolve(savedUser).then(savedUser => {
...
async function accessCachedKeys(query) {// Query is a json object of key-value pairs where
...
function closePage(currentURL) {
  ...
  tabQuery.then(tab => browser.tabs.remove(tab.id)).catch(error => {
...
async function openPage(pageId, message) {
  let pageIdFull = pageId + "-page"; //TODO: Format page if opening main page externally
  ...
  document.getElementById("main-title").innerHTML = pages[pageId].title;
...
function initializePage() {
  if (typeof document.body.dataset.internal !== "undefined" && document.body.dataset.internal === "true") {
    fetch("javascript-es6/pages.json").then(response => response.json()).then(data => {
      pages = data;
      openPage("login");
    }).catch(handleError);
  } else {
